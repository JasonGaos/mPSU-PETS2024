...
inline void psu2(std::vector<std::vector<u8>> inputSet_u8, std::vector<osuCrypto::block> inputSet_block, u64 nParties, u64 myIdx, u64 setSize, std::vector<std::vector<Channel>> chls)
{
	...
	auto end_offline = timer.setTimePoint("end offline");
	...
	for (u64 round = 1; round < nParties; round++)
	{
		if (myIdx == 0)
		{
			...
			for (u64 i = 0; i < simple.items.size(); i++)
			{
				for (u64 j = 0; j < simple.items[i].size(); j++)
				{
					...
					std::vector<osuCrypto::block> oprf_value = dh_oprf(pubHash, curve, 0, oprf_input, chlsoprf);
		...
		else if (myIdx == round)
		{
			...
			for (u64 i = 0; i < simple.items.size(); i++)
			{
				for (u64 j = 0; j < simple.items[i].size() + round - 1; j++)
				{
					...
					// simple
					// if(j<maxBinSize){
					// 	simple_new.items[i][j]= dh_prf({simple.items[i][j]}, key)[0];
					// }
				}
				// cuckoo
		...
		else if (myIdx > round && myIdx < nParties)
		{
			...
			setup_semi_honest(io, 1);
			std::vector<std::array<osuCrypto::block, 2>> aes_keys = rpir_batched_sender(chlsrpir, cuckoo.items, maxBinSize, io, &gc_sent, &gc_recv);
			...
			for (u64 i = 0; i < cuckoo.items.size(); i++)
			{
				...
				// should also update the Simple table???
			}
			// set update
			// recv rerand ctx
...
inline void psu2_o_t(std::vector<std::vector<u8>> inputSet_u8, std::vector<osuCrypto::block> inputSet_block, u64 nParties, u64 myIdx, u64 setSize, std::vector<std::vector<Channel>> chls)
{
	...
	std::cout << "party " << myIdx << " begin online execution" << std::endl;
	...
	for (u64 round = 1; round < nParties; round++)
	{
		...
		if (myIdx == 0)
		{
			...
			// simple.clear_table();
			// // oprf receiver
			// std::cout<<"party "<<myIdx<<" begin oprf"<<std::endl;
			// for (u64 i = 0; i < oprf.size(); i++)
			// { // oprf
			// 	std::cout<<"party "<<myIdx<<" oprf "<<i<<std::endl;
			// 	oprf[i] = dh_oprf(pubHash, curve, 0, {oprf[i]}, chlsoprf)[0];
			// 	// hash to simple table
			// 	for (u64 j = 0; j < idx_bin[i].size(); j++)
			// 	{
			...
			// auto end_oprf = timer.setTimePoint("end oprf ");
			// std::cout<<"party "<<myIdx<<" end oprf"<<std::endl;
		...
		else if (myIdx == round)
		{
			...
			std::vector<std::thread> pThrds(nParties - round);
			std::cout << pThrds.size() << std::endl;
			...
			for (u64 pIdx = 0; pIdx < pThrds.size(); ++pIdx)
			{
				pThrds[pIdx] = std::thread([&, pIdx]()
									...
									//    psu1(inputSet_u8[pIdx], inputSet_block[pIdx], nParties, pIdx, setSize, chls);
									    //   psu2_o_t(inputSet_u8[pIdx], inputSet_block[pIdx], nParties, pIdx, setSize, chls);
									      psu2_o(inputSet_u8[pIdx], inputSet_block[pIdx], nParties, pIdx, setSize, chls); 
										});
			}
