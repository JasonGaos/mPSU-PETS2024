...
// batched rpir
// return aes key
inline std::vector<osuCrypto::block> rpir_batched_receiver(std::vector<std::vector<Channel>> chls, std::vector<std::vector<osuCrypto::block>> inputSet)
{
	...
	for (u64 i = 0; i < inputSet.size(); i++)
	{
		...
		std::vector<block> input = inputSet[i];
...
inline std::vector<std::array<osuCrypto::block, 2>> rpir_batched_sender(std::vector<std::vector<Channel>> chls, std::vector<osuCrypto::block> inputSet, u64 maxBinSize)
{
	// sender
	AES hashOKVS(toBlock(12138));
...
{ // input is simple hash table (first 128 bit of each element)
	std::vector<osuCrypto::block> aes_keys = rpir_batched_receiver(chls, input);
	...
	for (u64 i = 0; i < input.size(); i++)
	{
		...
		block indicator = decryptor.ecbDecBlock(recv_aes_message[0]);
		std::vector<block> point_block;
...
{ // receive 2*16 bytes of coprf value to update inputSet_block
	...
	std::vector<std::array<osuCrypto::block, 2>> aes_keys = rpir_batched_sender(chls, input, 20);
	...
	for (u64 i = 0; i < input.size(); i++)
	{
		...
		// H(q)^alpha
		// Buuuuuuuuuuuuuuuuuuuugs with relic
		// v1 = v1 * alpha;
...
inline void psu2_coprf_test(std::vector<std::vector<u8>> inputSet_u8, std::vector<block> inputSet_block, u64 nParties, u64 myIdx, u64 setSize, std::vector<std::vector<Channel>> chls)
{
	u64 maxBinSize = 20;
...
// MPSU
inline void psu1_framework_withHash_batched(std::vector<std::vector<u8>> inputSet_u8, std::vector<block> inputSet_block, u64 nParties, u64 myIdx, u64 setSize, std::vector<std::vector<Channel>> chls)
{
	//=============================   Local Execution   ================================
	u64 maxBinSize = 20;
	// std::cout<<IoStream::lock;
	...
	for (u64 round = 1; round < nParties; round++)
	{
		...
		if (myIdx == 0)
		{
			...
			std::vector<block> aes_keys = rpir_batched_receiver(chlsrpir, simple.items);
		...
		else if (myIdx == round)
		{
			...
			std::vector<std::array<osuCrypto::block, 2>> aes_keys = rpir_batched_sender(chlsrpir, cuckoo.items, maxBinSize);
...
inline void psu1_final(std::vector<std::vector<u8>> inputSet_u8, std::vector<block> inputSet_block, u64 nParties, u64 myIdx, u64 setSize, std::vector<std::vector<Channel>> chls)
{
	//=============================   Local Execution   ================================
	u64 maxBinSize = 20;
	// std::cout<<IoStream::lock;
	...
	{ //----------------- with P1 -----------------
		...
		std::vector<osuCrypto::block> oprf_key = {prngOPRF.get<block>()};
		oprf_value = aes_oprf(1, inputSet_block, 2 * setSize, chlsoprf, oprf_key[0]);
		inputSet_block = oprf_value;
		//------------------ with others ----------------
		// 4.OPRF key sharing
	...
	for (u64 round = 1; round < nParties; round++)
	{
		...
		else if (myIdx == round)
		{
			...
			std::vector<std::array<osuCrypto::block, 2>> aes_keys = rpir_batched_sender(chlsrpir, cuckoo.items, maxBinSize + round - 1);
	...
	if (myIdx == 0)
	{
		std::vector<block> set_V_block;

		for (u64 i = 0; i < set_V.size(); i++)
		{
			// std::vector<std::vector<u8>> ctx = partial_decryption(set_V[i], s_keys[myIdx]);
			std::vector<block> ctx_block = ciphertexts_to_blocks(set_V[i]);
		...
		std::vector<block> recv_set_V_block(((nParties - 1) * tablesize) * 5);
